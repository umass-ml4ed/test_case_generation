...
def main():
    ...
    for group, code_values in group_wise_data.items():
        ...
        for prompt, code_info in zip(prompts, expt_code):
            ...
            buggy_compiler_code = procure_compiler_code(group, code_info[0], 1, code_info[1])
            correct_compiler_code = procure_compiler_code(group, correct_code, 1, code_info[1])
            ...
            output_1 = execute_and_store(group, 'p_{:d}'.format(code_info[1]))
            output_2 = execute_and_store(group, 'p_{:d}'.format(q))
            # compare outputs
