...
def create_initial_prompts(problem, correct_code, expt_code, q, input_type):
    ...
    for code, p in expt_code:
        ...
        system += 'Example: Code Input Parameters - int[] nums, String[] names. Output: Test Case Dictionary:  {"Explanation":"Write a very short explanation/ rationale behind generating/ editing test cases.", "test_case_1": "new int[]{1, 2, 3}, new String[]{"String1", "String2", "String3"}", "test_case_2": "new int[]{4, 5, 6}, new String[]{"NewString1", "NewString2", "NewString3"}"}'
...
def create_next_prompt(output_1, output_2, match_output, p, q, timeout_error):
    ...
    if not timeout_error:
        ...
        prompt = 'FEEDBACK: Results Table for every test case: (Yes denotes output match, No denotes output do not match)'
        prompt += table
        prompt += '\nREAL EXPECTED Test Case Result Matching: {:d}/{:d}'.format(p, q)
        prompt += '\nYOUR PREDICTED Test Case Result Matching: {:d}/{:d}'.format(new_match_output.count('Yes'), q)
        prompt += '\nVERY VERY IMPORTANT: MODIFY the test cases according to the FEEDBACK (such that YOUR PREDICTED TEST CASES satisfy the REAL EXPECTED Test Case Matching). Your Explanation should reflect the changes you made in the test cases wherever necessary.'
        prompt += '\n\nTest Case Dictionary:'
    else:
        ...
        prompt += '\nREAL EXPECTED Test Case Result Matching: {:d}/{:d}'.format(p, q)
        prompt += '\nVERY VERY IMPORTANT: MODIFY the test cases such that YOUR PREDICTED TEST CASES satisfy the REAL EXPECTED Test Case Matching. Your Explanation should reflect the changes you made in the test cases wherever necessary.'
        prompt += '\n\nTest Case Dictionary:'
    return prompt
